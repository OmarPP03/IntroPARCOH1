...
int main(){
  ...
  for(int i = 0; i < SIZE; i++){                                
    ...
    if(a[i] == NULL){
      printf("Memory not allocated (rows).");
      return EXIT_FAILURE;
    }
  }
  ...
  */ 
  ...
  printf("\n Elapsed time for sym check: %.9f seconds.\n", elapsed);
  ...
  printf(" Elapsed time for transpose operation: %.9f seconds.\n", elapsed);
...
float** matTranspose(float** matrix, int size){
  ...
  for(int i = 0; i < size; i++){
    ...
    if (trans[i] == NULL){
      printf("Memory not allocated for transposed matrix row number %d.", i);
      return NULL;
    }
  }
